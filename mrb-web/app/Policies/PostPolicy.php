...
class PostPolicy
{
    ...
    public function view(AuthUser $authUser, Post $post): bool
    {
        ...
        if ($authUser->hasRole('author')) {
            return $authUser->can('View:Post') && $post->users_id === $authUser->id;
        }

        // Super admin and editor can view all posts
    ...
    {
        if ($authUser->hasRole('author') && $authUser->id === $post->users_id) {
    ...
    public function delete(AuthUser $authUser, Post $post): bool
    {
        ...
        if ($authUser->hasRole('author')) {
            return $authUser->can('Delete:Post') && $post->users_id === $authUser->id;
        }

        // Super admin and editor can Delete all posts
