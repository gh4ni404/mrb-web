/**
 * Masjid Raya Baiturrahman Banda Aceh - Homepage Interaction Script
 * Features:
 * - Dynamic Island Navbar scroll behavior (simplified CSS class toggling)
 * - Viewport Intersection Observer for smooth scroll-driven page reveals
 * - Mobile navigation menu toggle
 * - Live Prayer Times, current active highlighting, and exact countdown
 * - Gregorian & Hijri calendar estimation
 * - Searchable and filterable Khutbah Friday Archive
 * - Interactive Tab-filtered Gallery with fade animation
 * - ZISWAF donation selector and Bank Account clipboard copy
 * - Testimonial slider rotating animation
 */
...
function initNavbar() {
  ...
  // Add scroll event listener to toggle Dynamic Island view via single class
...
function initPrayerWidget() {
  ...
  // Define static coordinates prayer times for Banda Aceh (approximate average)
  const prayerTimes = [
    { name: 'Subuh', time: '05:02' },
    { name: 'Syuruq', time: '06:22' },
    { name: 'Dzuhur', time: '12:38' },
    { name: 'Ashar', time: '16:04' },
    { name: 'Maghrib', time: '18:49' },
    { name: 'Isya', time: '20:04' }
  ];

  // Map elements inside DOM
  const prayerRows = {
  ...
  function updateTimesAndCountdown() {
    ...
    // 2. Determine Current Active and Next Prayer
    const currentTimeMs = now.getHours() * 3600000 + now.getMinutes() * 6000 + now.getSeconds() * 1000;
    ...
    // Convert prayer times to milliseconds since midnight
    const timesInMs = prayerTimes.map(p => {
      const [h, m] = p.time.split(':').map(Number);
      return {
        name: p.name,
        timeMs: h * 3600000 + m * 60000
      };
    });

    // Find current active prayer (the latest one that has passed)
    for (let i = 0; i < timesInMs.length; i++) {
      if (currentTimeMs >= timesInMs[i].timeMs) {
        activePrayer = timesInMs[i].name;
      }
    }
    // Handle late night after Isya
    if (!activePrayer) {
      activePrayer = 'Isya';
    }
  ...
  function getEstimatedHijriDate(date) {
    ...
    const baseGregDate = new Date(2026, 5, 5); // June 5, 2026
    const diffDays = Math.floor((date - baseGregDate) / 86400000);
    
    let day = 19 + diffDays;
    let month = 12; // Dzulhijjah
    let year = 1447;
...
function initDonationWidget() {
  ...
  const donationData = {
    zakat: {
      account: '7000 1111 88',
      name: 'Bank Syariah Indonesia (BSI)',
      holder: 'LAZIS Masjid Raya Baiturrahman Zakat'
    },
    infaq: {
      account: '7000 2222 99',
      name: 'Bank Syariah Indonesia (BSI)',
      holder: 'LAZIS Masjid Raya Baiturrahman Infaq'
    },
    sedekah: {
      account: '7000 3333 00',
      name: 'Bank Syariah Indonesia (BSI)',
      holder: 'LAZIS Masjid Raya Baiturrahman Sedekah'
    },
    wakaf: {
      account: '7000 4444 11',
      name: 'Bank Syariah Indonesia (BSI)',
      holder: 'Badan Wakaf Baiturrahman Banda Aceh'
    }
  };
    ...
    card.addEventListener('click', () => {
      donTypes.forEach(c => {
        c.classList.remove(
          ...
          'text-emerald-dark', 'border-transparent', 'shadow-lg', 
          'shadow-gold/25', 'scale-105'
        );
        c.classList.add(
